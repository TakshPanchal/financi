...
const getTransactionById = (req, res) => {
    const id = req.params.id
    pool.query('SELECT t.date, t.merchant_name, t.amount, t.closing_balance, t.tag_id, t.type, t.reference_number,t.transaction_id FROM users u JOIN transactions t ON u.user_id = t.user_id WHERE u.uid=$1',
        [id], (error, results) => {
        if (error) {
...
const getmonthlyTransactionById = (req,res) =>{
    const id = req.params.id
    const month1 = req.params.neededmonth //02
    ...
    var compare = `${format1}` > `2023-${month1}-01`  ? `2023-${month1}-01` : `${format1}`;


    let temp = new Date(compare);

    pool.query(`SELECT SUM(t.amount) FROM users u JOIN transactions t ON u.user_id = t.user_id WHERE t.date>='2023-${month1}-01' and t.date<=$1 and u.uid=$2 `,
    [temp,id], (error, results) => {
    if (error) {
...
const getmonthlyTransactioncountById = (req,res) =>{
    const id = req.params.id
    const month1 = req.params.neededmonth //02
    let objectDate = new Date();
    ...
    pool.query(`SELECT COUNT(t.merchant_name) FROM users u JOIN transactions t ON u.user_id = t.user_id WHERE t.date>='2023-${month1}-01' and t.date<=$1 and u.uid=$2 `,
    [temp,id], (error, results) => {
    if (error) {
        res.status(404).send('The Monthly transactions count for the given user does not exist')
        throw error
    }
    res.status(200).json(results.rows)
})
...
module.exports = {
    ...
    getmonthlyTransactioncountById
  }